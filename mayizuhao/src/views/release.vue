		...
		<div class="release-main white-radius wh-1300">
			...
			<div class="release-sel form-label-12">
				...
				<ul class="sel-list">
					...
					<li v-if="gameEGoods.length > 0">
						<el-select v-model="gameEGoodsPath" @change="selGameEs" size="medium" placeholder="请选择">
			...
			<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="form-label-12">
				...
				<div class="release-bar">
					...
					<h3>确认基础信息<span>为了避免帐号造成的问题，请核对相关信息，确保正确无误哦~</span></h3>
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.$api.post('AddGood', {
						...
						this.$notify({
							title: '温馨提示',
							message: '添加成功，请继续完善相关信息'
						});
						this.$router.push({path: '/release-change', query: {goodsCode: res.obj}});
					})
				}
			})
		},
