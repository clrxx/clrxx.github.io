<template>
	<div class="goods-released bc-f5f7f9">
		<div class="goods-released-main white-radius wh-1300">
			<div class="goods-released-bar"><h3>填写发布商品的信息资料</h3></div>
			<div class="goods-released-sel form-label-12">
				<label>选择游戏区服</label>
				<el-select v-model="gameVal" size="small" placeholder="请选择游戏类型">
					<el-option
			...
			<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="form-label-12">
				...
				<el-form-item label="图片上传" prop="uploadImg">
					<el-upload
						...
						<i class="el-icon-plus"></i>
  						<div slot="tip" class="el-upload__tip">已上传图片<span class="color-success">{{ ruleForm.uploadImg.length }}</span>/10张，每张图片最大支持1M，支持jpg/png格式</div>
					</el-upload>
				...
				<div class="goods-released-bar"><h3>商品价格设置</h3></div>
				<el-form-item label="出租单价" prop="unitPrice">
					<el-input v-model="ruleForm.unitPrice" size="small">
						<template slot="append">元/小时</template>
					</el-input>
				</el-form-item>
				<el-form-item label="包天" prop="allDay">
					<el-input v-model="ruleForm.allDay" size="small">
						<template slot="append">元/天</template>
					</el-input>
				</el-form-item>
				<el-form-item label="起租时间" prop="timeLen">
					<el-input v-model.number="ruleForm.timeLen" size="small">
				...
				<el-form-item prop="agreement">
					<el-checkbox v-model="ruleForm.agreement">我已经阅读相关协议<router-link to="/" class="color-success">《蚂蚁虚拟资产发布协议》</router-link></el-checkbox>
				</el-form-item>
...
export default {
	data () {
		...
		let validateAllDay = (rule, value, callback) => {
			if (value === '') {
		...
		return {
			...
			rules: {
				...
				allDay: [
					{ required: true, validator: validateAllDay, trigger: 'blur' }, 
				],
				timeLen: [
	...
	methods: {
		// datePickerChange (e) {
		// 	this.ruleForm.time[0] = this.$formatDateTime(e[0]);
		// 	this.ruleForm.time[1] = this.$formatDateTime(e[1]);
		// },
		beforeUpload (file) {
			console.log(file);
