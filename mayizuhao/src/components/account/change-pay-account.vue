		...
		<div class="account-puc">
			<div class="change-pass-main">
				...
				<el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="form-label-12">
					...
					<el-form-item label="新支付帐号" prop="pass">
						<el-input v-model="ruleForm.pass" size="small"></el-input>
					</el-form-item>
					<el-form-item label="确认新帐号" prop="checkPass">
						<el-input v-model="ruleForm.checkPass" size="small"></el-input>
					</el-form-item>
					<el-form-item>
...
export default {
	data() {
		...
		let validatePass2 = (rule, value, callback) => {
			if (value === '') {
				callback(new Error('请再次输入新帐号'));
			} else if (value !== this.ruleForm.pass) {
				callback(new Error('两次新账号输入不一致!'));
			} else {
				callback();
			}
		};
